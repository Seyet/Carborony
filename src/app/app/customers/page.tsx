...
export default async function CustomersPage({ searchParams }: CustomersPageProps) {
  ...
  return <div className="space-y-6"><header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs text-muted-foreground">CRM / Customers</p><h1 className="mt-2 text-2xl font-semibold tracking-tight sm:text-3xl">Customers</h1><p className="mt-1 text-sm text-muted-foreground">Understand every customer relationship and purchase history.</p></div><ButtonLink href="/app/customers/new"><Plus aria-hidden="true" />Add customer</ButtonLink></header>
    <CustomerSegmentCards currencyCode={customers.currencyCode} metrics={customers.segmentMetrics} />
    <Form action="/app/customers" className="grid gap-2 lg:grid-cols-[minmax(0,1fr)_220px_auto_auto]"><div className="relative"><Search aria-hidden="true" className="pointer-events-none absolute top-1/2 left-3 size-4 -translate-y-1/2 text-muted-foreground" /><Input className="h-10 pl-9" defaultValue={query} maxLength={100} name="query" placeholder="Search name, phone, email, address, or tag" type="search" /></div><select className="h-10 rounded-lg border border-input bg-background px-3 text-sm" defaultValue={segment} name="segment"><option value="">All segments</option>{customerSegments.map((item) => <option key={item} value={item}>{customerSegmentLabels[item]}</option>)}</select><Button className="h-10" type="submit"><Search aria-hidden="true" />Filter</Button>{hasFilters ? <ButtonLink className="h-10" href="/app/customers" variant="outline"><X aria-hidden="true" />Clear</ButtonLink> : null}</Form>
    {customers.items.length ? <><CustomersTable currencyCode={customers.currencyCode} items={customers.items} timezone={customers.timezone} /><div className="flex flex-col gap-3 text-sm text-muted-foreground sm:flex-row sm:items-center sm:justify-between"><p>Showing {(customers.page - 1) * customersPageSize + 1}–{Math.min(customers.page * customersPageSize, customers.totalCount)} of {customers.totalCount} customers</p><div className="flex items-center gap-2">{customers.page > 1 ? <ButtonLink href={customersHref(customers.page - 1, query, segment)} size="sm" variant="outline"><ChevronLeft aria-hidden="true" />Previous</ButtonLink> : <Button disabled size="sm" variant="outline"><ChevronLeft aria-hidden="true" />Previous</Button>}<span className="px-2">Page {customers.page} of {customers.pageCount}</span>{customers.page < customers.pageCount ? <ButtonLink href={customersHref(customers.page + 1, query, segment)} size="sm" variant="outline">Next<ChevronRight aria-hidden="true" /></ButtonLink> : <Button disabled size="sm" variant="outline">Next<ChevronRight aria-hidden="true" /></Button>}</div></div></> : <EmptyState action={hasFilters ? <ButtonLink href="/app/customers" variant="outline">Clear filters</ButtonLink> : <ButtonLink href="/app/customers/new">Add customer</ButtonLink>} description={hasFilters ? "No customers match the selected filters." : "Customers created here or captured from sales and orders will appear in this directory."} icon={Users} title={hasFilters ? "No matching customers" : "No customers yet"} />}
  </div>
}
