...
export default async function StorePage({ params, searchParams }: StorePageProps) {
  ...
  return <StorefrontShell preview={preview} store={store}>
    <main>
      ...
      >
        <div className="storefront-orbit pointer-events-none absolute -top-28 -left-28 size-72 rounded-full border border-foreground/5" />
        <div className="storefront-orbit storefront-orbit-reverse pointer-events-none absolute -top-16 -left-16 size-48 rounded-full border border-foreground/5" />
        <div className="mx-auto grid min-h-[590px] max-w-7xl gap-10 px-4 py-14 sm:px-6 sm:py-20 lg:grid-cols-[minmax(0,0.9fr)_minmax(420px,0.75fr)] lg:items-center lg:px-8 lg:py-24">
          <div className="storefront-hero-copy relative z-10">
          ...
          {store.settings.heroBannerUrl ? (
            ...
            <div className="storefront-hero-visual relative hidden aspect-[4/5] overflow-hidden rounded-[2rem] bg-foreground text-background shadow-2xl lg:flex lg:items-center lg:justify-center">
      ...
      <section className="border-b bg-card">
        <div className="storefront-stagger storefront-trust-strip mx-auto grid max-w-7xl divide-y px-4 sm:grid-cols-3 sm:divide-x sm:divide-y-0 sm:px-6 lg:px-8">
          <div className="storefront-trust-item flex items-center gap-3 py-5 sm:px-5 sm:first:pl-0"><span className="flex size-10 shrink-0 items-center justify-center rounded-full bg-emerald-500/10 text-emerald-700"><CheckCircle2 aria-hidden="true" className="size-5" /></span><span><strong className="block text-sm">{store.settings.copy.trustOneTitle}</strong><span className="text-xs text-muted-foreground">{store.settings.copy.trustOneDescription}</span></span></div>
          <div className="storefront-trust-item flex items-center gap-3 py-5 sm:px-5"><span className="flex size-10 shrink-0 items-center justify-center rounded-full bg-blue-500/10 text-blue-700"><Truck aria-hidden="true" className="size-5" /></span><span><strong className="block text-sm">{store.settings.copy.trustTwoTitle}</strong><span className="text-xs text-muted-foreground">{store.settings.copy.trustTwoDescription}</span></span></div>
          <div className="storefront-trust-item flex items-center gap-3 py-5 sm:px-5 sm:last:pr-0"><span className="flex size-10 shrink-0 items-center justify-center rounded-full bg-violet-500/10 text-violet-700"><ShoppingBag aria-hidden="true" className="size-5" /></span><span><strong className="block text-sm">{store.settings.copy.trustThreeTitle}</strong><span className="text-xs text-muted-foreground">{store.settings.copy.trustThreeDescription}</span></span></div>
        </div>
      </section>
