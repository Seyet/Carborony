import { CalendarDays, CircleAlert, CircleDollarSign, History, ShoppingBag, UserRound } from "lucide-react"
    ...
    <div className="space-y-6">
      ...
      <div className="grid items-start gap-6 xl:grid-cols-[minmax(0,1fr)_340px]">
        ...
        <div className="space-y-4">
          ...
          <Card><CardHeader><CardTitle>Order</CardTitle></CardHeader><CardContent className="space-y-3 text-sm"><div className="flex items-center gap-3"><CalendarDays aria-hidden="true" className="size-4 text-muted-foreground" />{dateTime(order.placedAt)}</div><div className="flex items-center gap-3"><ShoppingBag aria-hidden="true" className="size-4 text-muted-foreground" /><span className="capitalize">{order.channel}</span></div><div className="flex items-center gap-3"><CircleDollarSign aria-hidden="true" className="size-4 text-muted-foreground" /><span className="capitalize">Payment: {order.paymentStatus}</span></div>{order.verifiedPaymentStatus === "review" ? <div className="rounded-lg border border-amber-500/30 bg-amber-500/10 p-3 text-amber-900 dark:text-amber-200"><p className="flex items-center gap-2 font-medium"><CircleAlert aria-hidden="true" className="size-4" />Payment review required</p><p className="mt-1 text-xs leading-5">{order.paymentReviewReason ?? "The verified payment needs review before this order can be processed."}</p></div> : null}<div className="flex items-center justify-between border-t pt-3"><span className="text-muted-foreground">Status</span><Badge className={orderStatusClass(order.status)} variant="secondary">{orderStatusLabels[order.status]}</Badge></div></CardContent></Card>
