...
function money(currencyCode: string, value: number, compact = false) {
...
function buildMetrics(data: DashboardData, isToday: boolean): DashboardMetric[] {
  ...
  return [
    { title: salesLabel, value: money(data.currencyCode, metrics.salesTotal), helper: relativeChange(metrics.salesTotal, previousMetrics.salesTotal), icon: CircleDollarSign, tone: metricTone(metrics.salesTotal, previousMetrics.salesTotal) },
    { title: ordersLabel, value: integer(metrics.ordersCount), helper: relativeChange(metrics.ordersCount, previousMetrics.ordersCount), icon: ClipboardList, tone: metricTone(metrics.ordersCount, previousMetrics.ordersCount) },
    { title: expensesLabel, value: money(data.currencyCode, metrics.expensesTotal), helper: "Recorded expenses in selected period", icon: ReceiptText, tone: "neutral" },
    { title: profitLabel, value: money(data.currencyCode, metrics.profitTotal), helper: metrics.salesTotal ? `${((metrics.profitTotal / metrics.salesTotal) * 100).toFixed(1)}% estimated margin` : "Estimated after costs and expenses", icon: TrendingUp, tone: metrics.profitTotal < 0 ? "danger" : "positive" },
    { title: "Customers", value: integer(metrics.customersCount), helper: "Customers recorded to date", icon: Users, tone: "neutral" },
    { title: "Active Products", value: integer(metrics.productsCount), helper: "Active catalogue listings", icon: Package, tone: "neutral" },
    { title: "Low Stock", value: integer(metrics.lowStockCount), helper: metrics.lowStockCount ? "Needs your attention" : "All tracked stock is healthy", icon: Boxes, tone: metrics.lowStockCount ? "warning" : "neutral" },
    { title: "Out of Stock", value: integer(metrics.outOfStockCount), helper: metrics.outOfStockCount ? "Restock when ready" : "No tracked items are out", icon: PackageX, tone: metrics.outOfStockCount ? "danger" : "neutral" },
  ]
}

function chartLabel(date: string, pointCount: number) {
  const options: Intl.DateTimeFormatOptions = pointCount <= 7 ? { weekday: "short" } : { day: "numeric", month: "short" }
  return new Intl.DateTimeFormat("en-NG", { ...options, timeZone: "UTC" }).format(new Date(`${date}T00:00:00Z`))
}

function SalesOverview({ currencyCode, periodLabel, sales }: { currencyCode: string; periodLabel: string; sales: DashboardData["salesOverview"] }) {
  ...
  return <Card className="border-0 shadow-sm ring-1 ring-foreground/8 xl:col-span-2">
    ...
    <CardContent>{sales.length ? <><div className="mb-6"><p className="text-xs font-medium text-muted-foreground">Total sales</p><p className="mt-1 text-2xl font-semibold tracking-tight tabular-nums">{money(currencyCode, total)}</p></div><div aria-label="Bar chart of completed sales" className="grid h-64 grid-cols-[3.25rem_1fr] gap-3" role="img"><div className="flex flex-col justify-between pb-7 text-right text-[10px] tabular-nums text-muted-foreground">{labels.map((value) => <span key={value}>{money(currencyCode, value, true)}</span>)}</div><div className="relative grid gap-2" style={{ gridTemplateColumns: `repeat(${sales.length}, minmax(0, 1fr))` }}><div className="pointer-events-none absolute inset-x-0 top-0 h-px bg-border" /><div className="pointer-events-none absolute inset-x-0 top-1/4 h-px bg-border/70" /><div className="pointer-events-none absolute inset-x-0 top-1/2 h-px bg-border/70" /><div className="pointer-events-none absolute inset-x-0 top-3/4 h-px bg-border/70" /><div className="pointer-events-none absolute inset-x-0 bottom-7 h-px bg-border" />{sales.map((point) => <div className="relative z-10 flex min-w-0 flex-col justify-end gap-2" key={point.label}><div className="group flex min-h-0 flex-1 items-end justify-center"><div className="relative w-full max-w-10 rounded-t-md bg-primary/45 transition-colors hover:bg-primary" style={{ height: `${Math.max((point.value / max) * 100, point.value ? 2 : 0)}%` }} title={`${chartLabel(point.label, sales.length)}: ${money(currencyCode, point.value)}`}><span className="absolute -top-6 left-1/2 hidden -translate-x-1/2 whitespace-nowrap rounded bg-foreground px-1.5 py-0.5 text-[9px] font-medium text-background group-hover:block">{money(currencyCode, point.value, true)}</span></div></div><span className="h-5 text-center text-[10px] font-medium text-muted-foreground sm:text-xs">{chartLabel(point.label, sales.length)}</span></div>)}</div></div></> : <EmptyState message="No completed sales were recorded for this period." />}</CardContent>
  </Card>
}
...
export function DashboardOverview({ businessName, dashboard, dateRangeLabel, endDate, startDate, todayDate }: DashboardOverviewProps) {
  ...
  return <div className="space-y-6"><header className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between"><div><div className="mb-2 flex items-center gap-2"><Badge className="bg-emerald-500/10 text-emerald-700 dark:text-emerald-400" variant="secondary">Live overview</Badge></div><h1 className="text-2xl font-semibold tracking-tight sm:text-3xl">Good morning</h1><p className="mt-1.5 text-sm text-muted-foreground">{isToday ? `Here’s how ${businessName} is performing today.` : `Here’s how ${businessName} performed for ${dateRangeLabel}.`}</p></div><div className="flex w-full flex-col gap-2 sm:flex-row sm:items-end xl:w-auto"><DashboardDateFilter endDate={endDate} startDate={startDate} todayDate={todayDate} /><DashboardExportButton endDate={endDate} startDate={startDate} /></div></header><section aria-labelledby="business-metrics-heading"><h2 className="sr-only" id="business-metrics-heading">Business metrics</h2><div className="grid grid-cols-2 gap-3 md:grid-cols-4">{metrics.map((metric) => <MetricCard key={metric.title} metric={metric} />)}</div></section><section aria-label="Sales and product performance" className="grid gap-6 xl:grid-cols-3"><SalesOverview currencyCode={dashboard.currencyCode} periodLabel={periodLabel} sales={dashboard.salesOverview} /><TopProducts currencyCode={dashboard.currencyCode} periodLabel={periodLabel} products={dashboard.topProducts} /></section><section aria-label="Recent transactions and business alerts" className="grid gap-6 xl:grid-cols-3"><RecentTransactions currencyCode={dashboard.currencyCode} timeZone={dashboard.timeZone} transactions={dashboard.transactions} /><BusinessAlerts alerts={dashboard.alerts} /></section></div>
}
